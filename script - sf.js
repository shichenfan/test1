...
var xVariablePreset = 'Mobility_&_Congestion'
var yVariablePreset = 'Reliability'
var rVariablePreset = 'Connectivity'
var colorVariablePreset = 'Multimodalism'
...
info.update = function (props) {
	...
	this._div.innerHTML = 'Currently viewing project ' + props.ProjectID + closeChartButton;
		previousProps = props;
	...
	$('#clearFilter').click(function(){
		...
		$.each(layers, function(i, layer){
			layer.setStyle({
        		...
        		color: getColorScale(csvMap[layer.feature.properties.ARCID][0]),
				opacity: 0.5,
        		fillOpacity: 0.5,
        		weight: 5
        	});
		...
		if (searchTerm.length > 1){
			...
			$.each(rows, function(i, row){
				...
				$.each(layers, function(i, layer){
					...
					if (searchHighlightIds.indexOf(layer.feature.properties.ARCID) > -1){
						layer.setStyle({
		            	//	color: getColorScale(csvMap[layer.feature.properties.ID][0]),
		            		color: getColorScale(csvMap[layer.feature.properties.ARCID][0]),
		            		opacity: 0.5,
			...
			$.each(searchHighlightIds, function(i, id){
				...
				//var layer = _.find(counters.getLayers(), function(layer){return layer.feature.properties.ID == id;})
								var layer = _.find(counters.getLayers(), function(layer){return layer.feature.properties.ARCID == id;})
layer.setStyle({
            		color: getColorScale(csvMap[id][0]),
            		opacity: 0.5,
            		weight: 5
		...
		$.ajax({ 
			...
			success: function(json) {
				...
				for (var j = layers.length - 1; j >= 0; j--) {
        			...
        			var id = layers[j].feature.properties.ARCID;
...
function resetPrevious(){
	if(typeof previousLayer !== 'undefined' && previousLayer !== null){
		...
		$('#'+previousLayer.feature.properties.ARCID.replace(/ /gi, "-")+'-row').closest('tr').removeClass('warning');
		...
		var color = typeof jenks[colorVariable] !== 'undefined' || typeof jenks[colorVariable](csvMap[previousLayer.feature.properties.ARCID][0][colorVariable]) !== 'undefined' ? jenks[colorVariable](csvMap[previousLayer.feature.properties.ID][0][colorVariable]) : defaultHex;
...
function getLayerById(id){
	...
	for (var j = layers.length - 1; j >= 0; j--) {
		...
		if (layers[j].feature.properties.ARCID === id ){
			return layers[j];
		}
	}
}
...
function resetMarker(marker){
	...
	marker.setStyle({
		...
		color: getColorScale(csvMap[marker.feature.properties.ARCID][0]),
// fillOpacity: 1.0,
		opacity: 0.5
	});
	// marker.setRadius(3);
